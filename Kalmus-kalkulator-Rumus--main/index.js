...
function diagonalRumusLuasBelahKetupat() {
    let luas = parseFloat(document.getElementById('input-luas-belahketupat-d').value)
    let diagonalDiket = parseFloat(document.getElementById('input-diagonal2l-belahketupat-d').value)
    let output3 = document.getElementById('output-belahketupat3')
    let output1 = document.getElementById('output-belahketupat1')
    let outputPenjelasan = document.getElementById('output-penjelasan')

    if(isNaN(luas) || isNaN(diagonalDiket)) {
        output3.innerHTML = "<h1>Masukkan nilai yang valid!</h1>"  // Error message
        outputPenjelasan.innerHTML = ""  // Clear explanation if invalid
        return
    }

    let diagonalDicari = (luas * 2) / diagonalDiket

    output3.innerHTML = `<h1>Diagonal : ${diagonalDicari}<h1>`
    outputPenjelasan.innerHTML = `Lihat Penjelasan`
    output1.innerHTML = `
        <p>Anda memasukkan nilai Luas = ${luas} dan Diagonal 2 = ${diagonalDiket}.</p>
        <p>Untuk menghitung Diagonal 1 yang belum diketahui, gunakan rumus:</p>
        <p><strong>Diagonal 1 = (2 × Luas) / Diagonal 2</strong></p>
        <p>Dengan memasukkan nilai:</p>
        <ul>
            <li>Luas = ${luas}</li>
            <li>Diagonal 2 = ${diagonalDiket}</li>
        </ul>
        <p>Kami mendapatkan:</p>
        <p><strong>Diagonal 1 = (2 × ${luas}) / ${diagonalDiket} = ${diagonalDicari}</strong></p>
        <p>Jadi, Diagonal 1 yang belum diketahui adalah ${diagonalDicari}.</p>
    `
}

function diagonalRumusSisiBelahKetupat() {
    let sisi = parseFloat(document.getElementById('input-sisi-belahketupat-d').value)
    let diagonalDiket = parseFloat(document.getElementById('input-diagonal2s-belahketupat-d').value)
    let output3 = document.getElementById('output-belahketupat3')
    let output1 = document.getElementById('output-belahketupat1')
    let outputPenjelasan = document.getElementById('output-penjelasan')

    if(isNaN(sisi) || isNaN(diagonalDiket)) {
        output3.innerHTML = "<h1>Masukkan nilai yang valid!</h1>"  // Error message
        outputPenjelasan.innerHTML = ""  // Clear explanation if invalid
        return
    }

    let diagonalDicari = Math.sqrt(sisi ** 2 - ((diagonalDiket / 2) ** 2)) * 2

    output3.innerHTML = `<h1>Diagonal = ${diagonalDicari}`
    outputPenjelasan.innerHTML = `Lihat Penjelasan`
    output1.innerHTML = `
        <p>Anda memasukkan nilai Sisi = ${sisi} dan Diagonal 2 = ${diagonalDiket}.</p>
        <p>Untuk menghitung Diagonal 1 yang belum diketahui, kita menggunakan rumus:</p>
        <p><strong>Diagonal 1 = √(Sisi² - (Diagonal 2 / 2)²) × 2</strong></p>
        <p>Dengan memasukkan nilai:</p>
        <ul>
            <li>Sisi = ${sisi}</li>
            <li>Diagonal 2 = ${diagonalDiket}</li>
        ...
        <p>Langkah-langkah perhitungannya adalah sebagai berikut:</p>
        <p>Diagonal 1 = √(${sisi}² - (${diagonalDiket} / 2)²) × 2</p>
        <p>Diagonal 1 = √(${sisi}² - ${Math.pow(diagonalDiket / 2, 2)}) × 2</p>
        <p>Diagonal 1 = √(${Math.pow(sisi, 2)} - ${Math.pow(diagonalDiket / 2, 2)}) × 2</p>
        <p>Diagonal 1 = √(${Math.pow(sisi, 2) - Math.pow(diagonalDiket / 2, 2)}) × 2</p>
        <p>Hasilnya adalah ${diagonalDicari}, yang merupakan panjang Diagonal 1 yang belum diketahui.</p>
    `;
}
